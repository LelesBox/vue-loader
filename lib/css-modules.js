...
function replaceStyleClass (cssStr, keygen) {
  var regx = /\.([^0-9\/-].*\w|[^0-9\/-])(?=(.*?{))/gi
  // 先排除import语句
  var importRegx = /@import.*?;/gi
  var imports = []
  var r
  while ((r = importRegx.exec(cssStr))) {
    imports.push(r[0])
  }
  cssStr = cssStr.split(importRegx).filter((item) => item.trim() !== '').join('\n')
  return imports.join('\n') + cssStr.replace(regx, (match, str, index, cssStr) => {
